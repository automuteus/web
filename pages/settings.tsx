...
export default function SettingsPage() {
    ...
    useEffect(() => {
        if (!user) { setGuilds({ key: "" }); setSettings({ key: "" }); setPresence({ key: "" }); return; }
    ...
    useEffect(() => {
        ...
        fetch("/api/settings/defaults", { signal: controller.signal })
            .then(async (res) => {
                if (!res.ok) return;
                const data = await res.json();
                if (!controller.signal.aborted && data && typeof data === "object" && !Array.isArray(data) && typeof data.language === "string") setDefaults(data);
            }).catch(() => undefined);
        return () => controller.abort();
    }, [user]);
    ...
    return <AppLayout title="Server settings - AutoMuteUs" metaDesc="View AutoMuteUs voice rules, delays, and settings for the servers you manage.">
        <div className={styles.page}>
            ...
            {status === "loading" ? <div className={styles.state} role="status">Checking your Discord login...</div> : !user ?
                <div className={styles.state}><h2>Choose how your crew plays</h2><p>Sign in with Discord to view settings for the servers you manage.</p><button className={styles.button} onClick={login}>Sign in with Discord</button></div> :
                <>
                    {list.error ? problem(list, () => setGuildRetry((n) => n + 1)) : !list.data ? <div className={styles.state} role="status">Loading your servers...</div> : list.data.length === 0 ?
                        <div className={styles.state}><h2>No servers you can manage</h2><p>Only servers where you are the owner or have the Administrator permission are shown here. If you were just given that role, refresh your server list.</p><button className={styles.button} onClick={() => setGuildRetry((n) => n + 1)}>Refresh servers</button></div> : <>
                            <div className={styles.toolbar}><div className={styles.selector}><label htmlFor="settings-guild">Discord server</label><select id="settings-guild" value={guild ? selected : ""} onChange={(e) => router.replace({ pathname: "/settings", query: e.target.value ? { guild: e.target.value } : {} }, undefined, { shallow: true })}><option value="">Select a server</option>{[...list.data].sort((a, b) => a.name.localeCompare(b.name)).map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}</select></div><button className={styles.button} disabled={busy} onClick={() => setRefresh((n) => n + 1)}>Refresh settings</button></div>
                            <p className={styles.notice}>You can view settings here now. To make changes, use <code>/settings</code> in Discord for the time being. Servers without saved settings show the defaults. Values that differ from the bot&apos;s defaults are marked <strong>Changed</strong>; <strong>Premium</strong> marks settings the bot applies only on premium servers.</p>
                            {!guild ? <div className={styles.state}><h2>{selected ? "Server unavailable" : "Select your server"}</h2><p>{selected ? "This server isn't one you own or administer. Choose another server above." : "Choose a server to see its current configuration."}</p></div> : <>
                                ...
                                {!bot.data && !bot.error ? <div className={styles.state} role="status">Checking for AutoMuteUs in this server...</div> : bot.data?.present === false ?
                                    <div className={styles.state}>
                                        ...
                                        <p>Invite the bot to {guild.name} to use it there. Once it has joined, come back and refresh to see its settings.</p>
                                        ...
                                        {current.error ? problem(current, () => setRefresh((n) => n + 1)) : !current.data ? <div className={styles.state} role="status">Loading settings...</div> : <SettingsView settings={current.data} defaults={defaults} />}
                                    </>}
                            </>}
                        </>}
                </>}
